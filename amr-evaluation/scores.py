...
for score in preds:
    ...
    if pr + rc > 0:
        f = 2*(pr*rc)/(pr+rc)
        print (score, '-> P:', "{0:.4f}".format(pr), ', R:', "{0:.4f}".format(rc), ', F:', "{0:.4f}".format(f))
    else: 
        print (score, '-> P:', "{0:.4f}".format(pr), ', R:', "{0:.4f}".format(rc), ', F: 0.00')

pr, rc, f = smatch.main(reentrancies_pred, reentrancies_gold, True)
print ('Reentrancies -> P:', "{0:.4f}".format(float(pr)), ', R:', "{0:.4f}".format(float(rc)), ', F:', "{0:.4f}".format(float(f)))
pr, rc, f = smatch.main(srl_pred, srl_gold, True)
print ('SRL -> P:', "{0:.4f}".format(float(pr)), ', R:', "{0:.4f}".format(float(rc)), ', F:', "{0:.4f}".format(float(f)))
